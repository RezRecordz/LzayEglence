...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'bot') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**Bak bot diyosun kalbimi kırıyosun atarım vallaha sunucudan**'); 
		} else {
		msg.reply('Bak bot diyosun kalbimi kırıyosun atarım vallaha sunucudan');
		}
	}
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'hayat nasıl') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**nasıl olsun iş güç koşturuyoz**'); 
		} else {
		msg.reply('nasıl olsun iş güç koşturuyoz');
		}
	}
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'nasılsın') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('iyi knk sen nasılsın'); 
		} else {
		msg.reply('iyi knk sen nasılsın');
		}
	}
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'lan bot acıktım') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('iyi knk sen nasılsın'); 
		} else {
		msg.reply('bokumu ye puşt');
		}
	}
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'acıktım') {
		...
		msg.reply('ne veriyim abime');
		}
	}
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'menü alabilirmiyim') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('iyi knk sen nasılsın'); 
		} else {
		msg.reply('hamburger var dürüm var çiğköfte var döner var ayvalık tostu var ne istersin abi  ');
		}
	}
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'hamburger olsun bi de kola') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('iyi knk sen nasılsın'); 
		} else {
		msg.reply('hemen geliyor efendim');
		}
	}
});
client.on("ready", async message => {
  client.channels.cache.get('786192372798062633').join()
});
...
client.login(ayarlar.token);
 client.on('ready', ()=>{
 client.channels.get("786181373622222868").join()
    }) 
//RezRecordz Kodlayıcıları Tarafından Yapıldı
